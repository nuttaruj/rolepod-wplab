...
describe('RestClient', () => {
  ...
  it('refuses non-https base URL', () => {
    expect(() => new RestClient({ baseUrl: 'http://example.com', credential: CRED })).toThrow(
      WplabError,
    )
  })

  it('strips trailing slash from base URL', async () => {
    const client = new RestClient({ baseUrl: 'https://example.com/', credential: CRED })
    fetchSpy.mockResolvedValue(jsonResponse(200, { ok: true }))
    await client.request({ path: '/wp/v2/types' })
    expect(fetchSpy.mock.calls[0]![0]).toBe('https://example.com/wp-json/wp/v2/types')
  })
  ...
  it('attaches Basic Authorization header with App Password', async () => {
    ...
    const expected = 'Basic ' + Buffer.from('admin:xxxx yyyy zzzz').toString('base64')
  ...
  it('falls back to ?rest_route= form on 404 with rest_no_route body', async () => {
    const client = new RestClient({ baseUrl: 'https://example.com', credential: CRED })
    fetchSpy
      .mockResolvedValueOnce(jsonResponse(404, { code: 'rest_no_route', message: 'No route' }))
      .mockResolvedValueOnce(jsonResponse(200, { ok: true }))

    const res = await client.request({ path: '/wplab/v1/handshake' })
    expect(res.status).toBe(200)
    expect(fetchSpy.mock.calls).toHaveLength(2)
    expect(fetchSpy.mock.calls[1]![0]).toContain('rest_route=%2Fwplab%2Fv1%2Fhandshake')
  })

  it('falls back to ?rest_route= form on 404 with HTML body', async () => {
    const client = new RestClient({ baseUrl: 'https://example.com', credential: CRED })
    fetchSpy
      .mockResolvedValueOnce(htmlResponse(404, '<!DOCTYPE html><html>not found</html>'))
      .mockResolvedValueOnce(jsonResponse(200, { ok: true }))
  ...
  it('encodes query parameters', async () => {
    const client = new RestClient({ baseUrl: 'https://example.com', credential: CRED })
    fetchSpy.mockResolvedValue(jsonResponse(200, []))
    await client.request({ path: '/wp/v2/posts', query: { per_page: 5, status: 'publish' } })
