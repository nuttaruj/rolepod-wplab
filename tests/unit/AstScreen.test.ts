...
import { screenPhpPayload, assertPhpPayloadOk } from '../../src/safety/AstScreen.js'
...
describe('AstScreen', () => {
  it('allows simple expressions', () => {
    ...
    expect(screenPhpPayload('$x = wp_get_current_user(); return $x->ID;').ok).toBe(true)
  })
  ...
  it('rejects system / exec / shell_exec / passthru / proc_open / popen', () => {
    for (const fn of ['system', 'exec', 'shell_exec', 'passthru', 'proc_open', 'popen']) {
  ...
  it('assertPhpPayloadOk throws AstRejectedError on bad input', () => {
    expect(() => assertPhpPayloadOk('system("ls");', 'execute_php')).toThrow(AstRejectedError)
