...
describe("createServer", () => {
  it("wires without binding stdio", async () => {
    const { server, registry, shutdown } = await createServer({
      withoutTransport: true,
    });
...
describe("safety/AllowList", () => {
  ...
  it("allows destructive subcommands when opt-in", () => {
    expect(checkWpCli(["plugin", "install", "jetpack"], true).allowed).toBe(
      true,
    );
    expect(
      checkWpCli(["option", "update", "siteurl", "https://x.test"], true)
        .allowed,
    ).toBe(true);
  });
...
describe("safety/ProdGuard", () => {
  ...
  it("enforce throws ProductionBlockedError on match", () => {
    const guard = new ProdGuard(["mysite.com"]);
    expect(() => guard.enforce("https://mysite.com")).toThrow(
...
describe("safety/FsScope", () => {
  ...
  it("allows wp-content/plugins write", () => {
    mkdirSync(join(wpRoot, "wp-content", "plugins", "sample"), {
      recursive: true,
    });
    writeFileSync(
      join(wpRoot, "wp-content", "plugins", "sample", "sample.php"),
      "",
    );
    const abs = resolveScopedWrite(
      wpRoot,
      "wp-content/plugins/sample/sample.php",
      false,
    );
  ...
  it("allows wp-config.php", () => {
    ...
    expect(() =>
      resolveScopedWrite(wpRoot, "wp-config.php", false),
    ).not.toThrow();
  });
  ...
  it("rejects wp-admin write without confirm", () => {
    ...
    expect(() =>
      resolveScopedWrite(wpRoot, "wp-admin/evil.php", false),
    ).toThrow(FsScopeError);
  });

  it("allows wp-admin with confirm_unsafe_path=true", () => {
    expect(() =>
      resolveScopedWrite(wpRoot, "wp-admin/explicit.php", true),
    ).not.toThrow();
  });

  it("rejects path traversal outside wpRoot", () => {
    expect(() => resolveScopedWrite(wpRoot, "../escape.txt", false)).toThrow(
      FsScopeError,
    );
  });
  ...
  it("rejects disallowed extension in uploads/", () => {
    expect(() =>
      resolveScopedWrite(wpRoot, "wp-content/uploads/payload.exe", false),
    ).toThrow(FsScopeError);
  });
