...
describe("canonicalizeSite", () => {
  it("lowercases hostname", () => {
    expect(canonicalizeSite("https://Walnutztudio.com/")).toBe(
      "walnutztudio.com",
    );
  });
  ...
  it("handles subdomains", () => {
    expect(canonicalizeSite("https://Staging.Site.Co.UK/admin")).toBe(
      "staging.site.co.uk",
    );
  });
...
describe("FileVault", () => {
  ...
  it("list returns metadata without secret", async () => {
    ...
    expect(
      (list[0] as unknown as { appPassword?: string }).appPassword,
    ).toBeUndefined();
  });
