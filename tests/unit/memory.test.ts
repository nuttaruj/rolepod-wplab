...
describe("MemoryStore", () => {
  ...
  it("ensureSite creates dir + meta.json at mode 0700/0600", async () => {
    await MemoryStore.ensureSite(
      "walnutztudio.com",
      "https://walnutztudio.com",
    );
    expect(await MemoryStore.exists("walnutztudio.com")).toBe(true);
    const meta = JSON.parse(
      readFileSync(join(tmp, "walnutztudio.com", "meta.json"), "utf8"),
    );
  ...
  it("appendNote (convention) appends with version marker", async () => {
    await MemoryStore.appendNote(
      "site.com",
      "use ACF Pro field group X",
      "convention",
    );
    await MemoryStore.appendNote(
      "site.com",
      "switched to Meta Box",
      "convention",
    );
    const content = readFileSync(
      join(tmp, "site.com", "conventions.md"),
      "utf8",
    );
  ...
  it("appendNote (runbook) requires runbook_name", async () => {
    await expect(
      MemoryStore.appendNote("site.com", "deploy steps", "runbook"),
    ).rejects.toThrow(/runbook_name/);
  });
  ...
  it("appendNote (runbook) writes to runbooks/<name>.md replacing previous", async () => {
    ...
    const content = readFileSync(
      join(tmp, "site.com", "runbooks", "deploy.md"),
      "utf8",
    );
  ...
  it("recall filters by kind", async () => {
    ...
    const onlyConv = await MemoryStore.recall("site.com", {
      kind: "convention",
    });
  ...
  it("writeSiteSnapshot replaces site.md", async () => {
    ...
    expect(readFileSync(join(tmp, "site.com", "site.md"), "utf8")).toBe(
      "second snapshot",
    );
  });
});
