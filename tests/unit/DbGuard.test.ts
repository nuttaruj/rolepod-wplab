...
describe("DbGuard", () => {
  it("allows SELECT", () => {
    expect(() =>
      assertReadOnlyOrAllowed("SELECT * FROM wp_posts", false),
    ).not.toThrow();
  });
  ...
  it("allows DESCRIBE / DESC", () => {
    expect(() =>
      assertReadOnlyOrAllowed("DESCRIBE wp_posts", false),
    ).not.toThrow();
  ...
  it("allows EXPLAIN", () => {
    expect(() =>
      assertReadOnlyOrAllowed("EXPLAIN SELECT * FROM wp_posts", false),
    ).not.toThrow();
  });
  ...
  it("strips line comments before checking", () => {
    expect(() =>
      assertReadOnlyOrAllowed("-- a comment\nSELECT 1", false),
    ).not.toThrow();
  });

  it("strips block comments before checking", () => {
    expect(() =>
      assertReadOnlyOrAllowed("/* note */ SELECT 1", false),
    ).not.toThrow();
  });
  ...
  it("blocks INSERT by default", () => {
    expect(() =>
      assertReadOnlyOrAllowed(
        "INSERT INTO wp_posts (post_title) VALUES ('x')",
        false,
      ),
    ).toThrow(DbWriteBlockedError);
  });
  ...
  it("blocks DELETE by default", () => {
    expect(() =>
      assertReadOnlyOrAllowed("DELETE FROM wp_posts WHERE ID=1", false),
    ).toThrow(DbWriteBlockedError);
  });

  it("blocks DROP / TRUNCATE / ALTER", () => {
    expect(() => assertReadOnlyOrAllowed("DROP TABLE wp_posts", false)).toThrow(
      DbWriteBlockedError,
    );
    expect(() => assertReadOnlyOrAllowed("TRUNCATE wp_posts", false)).toThrow(
      DbWriteBlockedError,
    );
    expect(() =>
      assertReadOnlyOrAllowed("ALTER TABLE wp_posts ADD foo INT", false),
    ).toThrow(DbWriteBlockedError);
  });

  it("blocks empty / whitespace-only", () => {
    expect(() => assertReadOnlyOrAllowed("", false)).toThrow(
      DbWriteBlockedError,
    );
  ...
  it("allows anything when allowWrite=true", () => {
    ...
    expect(() =>
      assertReadOnlyOrAllowed("DROP TABLE wp_posts", true),
    ).not.toThrow();
  });
});
