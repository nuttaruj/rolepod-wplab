...
wrote. The only mark was one sentence in `readme.txt` — a file WordPress never
parses and the first thing anyone overwrites — and it covered `scaffold_plugin`
alone. A plugin written through `file_write`, or built locally and uploaded as
a zip, carried nothing at all.

### Added

- `src/lib/provenance.ts` — one stamp, used everywhere, so a single grep finds
  every artifact this server produced. Two marks, because they survive
  different things: `Generated-By: rolepod-wplab` + `Generated-At: <date>` in
  the header WordPress actually parses (travels with the directory when it is
  copied to another site; WordPress ignores headers it does not know), and
  `(generated by rolepod-wplab)` appended to `Description`, which is the half a
  human reads in wp-admin with no tooling at all.
- Both marks now ship from `rolepod_wp_scaffold_plugin`,
  `rolepod_wp_scaffold_theme`, `rolepod_wp_child_theme_create`,
  `rolepod_wp_cpt_scaffold` and `rolepod_wp_scaffold_pattern`.
- `SERVER_INSTRUCTIONS` gains **"Sign what you write"** — including the case the
  scaffolds cannot cover: a plugin assembled by hand or uploaded as a zip is
  unmarked unless the model adds the same two marks itself.
- `wp-scaffold` skill: Iron Rule 4 (never strip the stamp, add it by hand when
  you build a plugin yourself) and a Provenance section giving the read path —
  `rolepod_wp_file_read` with `max_bytes: 2048` on the bootstrap file, which
  works on a guarded site.
- `tests/unit/provenance.test.ts` — 10 cases, including that re-scaffolding does
  not stack the suffix and that the agent name merely mentioned in prose is not
  read as a provenance claim.

### Changed

- `rolepod_wp_cpt_scaffold` used its own wording ("Scaffolded by
  rolepod-wplab.") in the description. It now uses the shared suffix, so one
  string matches every artifact.

### Notes

- This is provenance, not attestation: it records what this server wrote, the
  lines can be deleted, and an unmarked plugin proves nothing. The change ledger
  (`rolepod_wp_changes_query`) is the counterpart a file edit cannot rewrite —
  per-site, where the header travels with the files.
- The stamp carries no version because the server has none to report:
  `src/server.ts` still announces `0.0.0` to MCP clients. No version is better
  than a wrong one; wiring the real version through is separate work.
