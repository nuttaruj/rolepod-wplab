...
**Standalone:** complete WordPress operations toolkit. 15 skills + 139 MCP tools
...
- **Audit wp-admin without anyone logging in.** `rolepod_wp_admin_one_time_link(target_id, destination)` mints a 5-minute single-use URL; hand it to rolepod-uiproof `browser_open`, or to any other browser automation on the machine (Chrome MCP, Playwright) — the link is a plain URL, so not having uiproof blocks nothing — and the whole browser session stays signed in as the admin. The AI says which it used, so nobody waits for a login prompt that never comes; a person clicking the link is the last resort. Whoever opens it holds a full admin session, and a HAR or trace recorded during it contains the auth cookie — treat those as credentials.
...
Skills are not auto-registered under Cursor (no unified plugin format yet). The 139 MCP tools are still available; invoke them by name in chat (`Use rolepod_wp_pair to …`).
...
Skills are not auto-registered under Gemini (no unified plugin format yet — `gemini-extension.json` is still in flux). The 139 MCP tools are still available; invoke them by name in chat (`Use rolepod_wp_pair to …`).
...
139 MCP tools (`rolepod_wp_*`) will appear in your client. Skills are not surfaced via this path — call the tools by name.
...
Path B works without the companion plugin — you get all 139 MCP tools EXCEPT `execute_php`, `introspect`, `hook_state`, `mail_test` (those need the companion).
...
- **139 MCP tools** — connectivity (4 kinds) + wp-cli passthrough + typed CRUD + page-builder adapters (Elementor / Divi / Oxygen / Bricks) + SEO/i18n (Yoast / RankMath / WPML) + WooCommerce + ACF + forms (Gravity / CF7 / WPForms) + cron + cache + mail + clone + backup + diagnose + scaffold (block / plugin / theme / pattern) + REST dump + user sessions + companion-gated (execute*php / introspect / hook_state) + pair (one-click setup). All prefixed `rolepod_wp*\*`.
