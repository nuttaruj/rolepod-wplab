...
3. ALWAYS surface every absolute path written + every backup file created — the only reliable rollback is to restore the `.wplab-bak-...` siblings or `rm -rf` the slug dir. Only the newest 2 backups per file survive (each write prunes the rest), so a rollback that has to go further back reads the Change Ledger instead.
4. ALWAYS leave the provenance stamp on. The scaffold tools add `Generated-By: rolepod-wplab` + `Generated-At: <date>` to the plugin/theme header and `(generated by rolepod-wplab)` to `Description` — do not strip either when editing the generated file, and add both BY HAND to any plugin or theme you assemble yourself with `file_write` or upload as a zip. A site owner inheriting the install has no other way to tell what an AI wrote.
</EXTREMELY-IMPORTANT>
...
Everything these tools write is signed twice, because the two marks survive
different things:

| Mark | Where | Survives |
| --- | --- | --- |
| `Generated-By: rolepod-wplab` + `Generated-At: <date>` | plugin main PHP header, or theme `style.css` header | copying the directory to another site; WordPress ignores headers it does not know |
| `(generated by rolepod-wplab)` appended to `Description` | same header | being read by a human in wp-admin → Plugins / Appearance → Themes, with no tooling at all |

To check a plugin later, read the top of its bootstrap file and look for the
`Generated-By` line:
...
Reads work on a guarded site, so this needs no Full Access.

**This is provenance, not attestation.** It records what rolepod-wplab wrote.
Anyone can delete the lines, and a plugin that arrived by any other route was
never marked — so an unmarked plugin proves nothing. The server-side change
ledger (`rolepod_wp_changes_query`) is the counterpart that editing a file
cannot rewrite; it is per-site, where the header travels with the files.
