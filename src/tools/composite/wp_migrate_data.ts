...
export async function wpMigrateDataHandler(
  ...
  await writeFile(
    reportPath,
    JSON.stringify({ run_id: runId, scope: input.scope, source: source.siteurl, dest: dest.siteurl, applied }, null, 2),
    'utf8',
  )
...
async function migratePluginVersions(source: Target, dest: Target): Promise<MigrateDataOutput['applied']> {
  const [srcList, dstList] = await Promise.all([pluginsOf(source), pluginsOf(dest)])
  ...
  for (const sp of srcList) {
    ...
    if (!dp) {
      ...
      const r = await dest.wpCli(['plugin', 'install', sp.name, '--version=' + sp.version, '--activate'], {
      ...
      const action: 'upgrade' | 'downgrade' = isNewer(sp.version, dp.version) ? 'upgrade' : 'downgrade'
      const r = await dest.wpCli(['plugin', 'install', sp.name, '--version=' + sp.version, '--force'], {
        allowDestructive: true,
      })
      applied.push({
        action,
      ...
      applied.push({ action: 'noop', slug: sp.name, from: dp.version, to: sp.version, ok: true })
    }
  }
...
async function pluginsOf(t: Target): Promise<Array<{ name: string; version: string }>> {
