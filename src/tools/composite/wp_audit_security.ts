...
export async function wpAuditSecurityHandler(
  ...
  const coreCheck = await target.wpCli([
    "core",
    "check-update",
    "--format=json",
  ]);
  const coreOutdated =
    coreCheck.exitCode === 0 &&
    coreCheck.stdout.trim() !== "" &&
    coreCheck.stdout.trim() !== "[]";
  ...
  const userList = await target.wpCli([
    "user",
    "list",
    "--role=administrator",
    "--format=json",
  ...
  if (userList.exitCode === 0) {
    try {
      const users = JSON.parse(userList.stdout || "[]") as Array<{
        user_login: string;
      }>;
      for (const u of users) {
        ...
        if (login === "admin") {
          weakAdmins.push({
            login: u.user_login,
            reason: 'default/easy-to-guess login name "admin"',
          });
        }
      }
    } catch {
  ...
  const wpDebugOn =
    wpDebug.exitCode === 0 && /^(1|true)\s*$/i.test(wpDebug.stdout);
...
function renderMarkdown(j: {
  ...
  for (const p of j.outdated_plugins)
    lines.push(`- ${p.slug}: ${p.current} → ${p.latest}`);
  lines.push("");
  lines.push(`## Outdated themes (${j.outdated_themes.length})`);
  for (const t of j.outdated_themes)
    lines.push(`- ${t.slug}: ${t.current} → ${t.latest}`);
  lines.push("");
