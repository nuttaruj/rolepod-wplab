...
export async function wpMigrateDryrunHandler(
  ...
  return MigrateDryrunOutputSchema.parse({
    run_id: runId,
    plan,
    plan_path: planPath,
  });
...
async function diffPlugins(source: Target, dest: Target): Promise<unknown> {
  const [s, d] = await Promise.all([pluginsOf(source), pluginsOf(dest)]);
...
async function pluginsOf(
  target: Target,
): Promise<Array<{ name: string; version: string; status: string }>> {
