...
export async function wpScaffoldBlockHandler(
  ...
  if (!ns || !slug) {
    throw new WplabError('SCAFFOLD_BLOCK_BAD_SLUG', 'block_slug must be "namespace/slug"', {})
  }
  ...
  const blockJson = {
    ...
    ...(input.render_strategy === 'dynamic' ? { render: 'file:./render.php' } : {}),
    attributes: {},
    supports: { html: false },
  }
  await target.fileWrite(`${blockDir}/block.json`, JSON.stringify(blockJson, null, 2), { backup: false })
...
`
    await target.fileWrite(`${blockDir}/render.php`, renderPhp, { backup: false })
  ...
  return ScaffoldBlockOutputSchema.parse({ run_id: runId, files_written: written, next_steps: nextSteps })
}
