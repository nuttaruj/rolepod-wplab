...
export const wpElementorValidateDataToolDef = {
  ...
  description:
    "Walk an `_elementor_data` JSON tree and verify every widget's settings against the live widget_schema on the target. Catches the silent type-mismatch class of bug (legacy `icon` control expects a string, not a {value,library} array — passing the wrong shape renders an empty `<i class=\"Array\">`). ALSO emits schema-free structural warnings: section-level `_css_classes` (free Elementor doesn't render those to the DOM) and block-level HTML inside an icon-box `description_text` (auto-closes the wrapping <p>). Schema is fetched once per widget type per call. Returns errors[] + warnings[]; call BEFORE you commit the JSON via _elementor_data meta so you don't ship broken pages. Requires rolepod-wp companion v2.11+ AND Elementor active.",
