...
export async function wpWooWriteHandler(
  ...
  if (matched.matched && !input.confirm) {
    throw new WplabError('PRODUCTION_BLOCKED', 'woo_write on prod target needs confirm=true', {
      siteurl: target.siteurl,
      matchedPattern: matched.pattern,
    })
  }
  if (!(await woocommerceAdapter.detect(target))) {
    throw new WplabError('ADAPTER_NOT_DETECTED', 'WooCommerce not active on target', {
  ...
  if (input.op === 'update_product') {
    if (input.product_id === undefined || input.fields === undefined) {
      throw new WplabError('WOO_WRITE_BAD_INPUT', 'update_product requires product_id + fields', {})
    }
    const result = await woocommerceWrite.updateProduct(target, input.product_id, input.fields)
  ...
  if (input.op === 'bulk_update_prices') {
    if (!input.price_updates || input.price_updates.length === 0) {
      throw new WplabError('WOO_WRITE_BAD_INPUT', 'bulk_update_prices requires non-empty price_updates', {})
    }
    const result = await woocommerceWrite.bulkUpdatePrices(target, input.price_updates)
    return WooWriteOutputSchema.parse({ op: input.op, result })
  }

  // exhaustive
  throw new WplabError('WOO_WRITE_UNKNOWN_OP', `Unknown op: ${String(input.op)}`, {})
}
