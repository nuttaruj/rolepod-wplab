...
export async function wpAcfWriteHandler(
  ...
  if (matched.matched && !input.confirm) {
    throw new WplabError('PRODUCTION_BLOCKED', 'acf_write on prod target needs confirm=true', {
      siteurl: target.siteurl,
      matchedPattern: matched.pattern,
    })
  }
  if (!(await acfAdapter.detect(target))) {
    throw new WplabError('ADAPTER_NOT_DETECTED', 'ACF not active on target', { targetId: input.target_id })
  }
  const result = await acfWrite.setPostMeta(target, input.post_id, input.field_name, input.value)
