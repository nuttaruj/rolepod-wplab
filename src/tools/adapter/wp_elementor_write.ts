...
export async function wpElementorWriteHandler(
  ...
  if (matched.matched && !input.confirm) {
    throw new WplabError('PRODUCTION_BLOCKED', 'elementor_write on prod target needs confirm=true', {
  ...
  if (!detected) {
    throw new WplabError('ADAPTER_NOT_DETECTED', 'Elementor not active on target', { targetId: input.target_id })
  }
  const result = await elementorWrite.updatePageData(target, input.post_id, input.widget_tree)
