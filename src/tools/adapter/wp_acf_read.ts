...
export async function wpAcfReadHandler(
  ...
  if (!detected) {
    return AcfReadOutputSchema.parse({ scope: input.scope, detected: false, items: [] })
  }

  switch (input.scope) {
    case 'field_groups': {
      const items = await acfAdapter.read.fieldGroups(target)
      return AcfReadOutputSchema.parse({ scope: input.scope, detected: true, items })
    }
    case 'fields_in_group': {
      if (!input.group_key) {
        throw new WplabError('ACF_READ_MISSING_GROUP_KEY', 'scope=fields_in_group requires group_key arg', {})
      }
      const items = await acfAdapter.read.fieldsInGroup(target, input.group_key)
      return AcfReadOutputSchema.parse({ scope: input.scope, detected: true, items })
    }
    case 'post_meta': {
      if (input.post_id === undefined) {
        throw new WplabError('ACF_READ_MISSING_POST_ID', 'scope=post_meta requires post_id arg', {})
      }
      const meta = await acfAdapter.read.postMeta(target, input.post_id)
      return AcfReadOutputSchema.parse({ scope: input.scope, detected: true, meta })
    }
  }
}
