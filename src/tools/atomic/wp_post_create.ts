...
export async function wpPostCreateHandler(
  ...
  if (res.status < 200 || res.status >= 300) {
    throw new WplabError('POST_CREATE_FAILED', `REST returned HTTP ${res.status}`, {
      status: res.status,
      body: res.body,
    })
  }
  ...
  if (typeof b.id !== 'number') {
    throw new WplabError('POST_CREATE_NO_ID', 'REST response missing post id', { body: res.body })
  }
