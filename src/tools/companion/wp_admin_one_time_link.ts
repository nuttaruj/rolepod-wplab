...
export const wpAdminOneTimeLinkToolDef = {
  ...
  description:
    ...
    "If an admin POST comes back 403 while every GET succeeds, that is a WAF rule on the browser's user-agent, not an authentication problem — the session is fine, and only writes are refused. rolepod-uiproof 0.20.0+ presents a desktop UA by default; older versions need an explicit one, and a host that still refuses may be inspecting Sec-CH-UA client hints, which a user-agent override does not rewrite. " +
