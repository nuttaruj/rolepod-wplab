...
\`rolepod_wp_child_theme_create\` is stamped automatically:
two header lines in the file WordPress parses —

    Generated-By: rolepod-wplab
    Generated-At: <YYYY-MM-DD>

— plus \`(generated by rolepod-wplab)\` appended to \`Description\`, which is the
half a human sees in wp-admin without any tooling.

**When you write one by hand, add the same two marks yourself.** A plugin
assembled with \`rolepod_wp_file_write\`, or built locally and uploaded as a zip,
carries nothing unless you put it there, and a site owner then has no way to
tell where it came from. Put the header lines in the main plugin PHP file (or a
theme's \`style.css\`) and the suffix on \`Description\`.

To check an existing plugin later, read the first couple of KB of its main PHP
file — \`rolepod_wp_file_read\` with \`max_bytes\` — and look for the
\`Generated-By\` line. Reads work on a guarded site. This is provenance, not
attestation: it records what this server wrote, and anyone can delete the lines,
so an unmarked plugin is not evidence of anything. The server-side ledger
