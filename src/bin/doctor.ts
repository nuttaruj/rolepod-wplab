...
export async function runDoctor(): Promise<number> {
  ...
  if (profile.production_hosts.length > 0) {
    ...
    lines.push(
      "  ! No production_hosts configured — set ROLEPOD_WPLAB_PROD_HOSTS or profile.json",
    );
  }

  if (profile.profile === "power") {
    lines.push(
      "  ! Power profile selected — companion tools land in v0.2 (not yet registered in v0.0)",
    );
  }
