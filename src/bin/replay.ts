...
export async function runReplay(argv: string[]): Promise<number> {
  ...
  process.stderr.write(
    `[replay] loaded ${bundle.calls.length} calls from ${path}\n`,
  );
  if (bundle.description)
    process.stderr.write(`[replay] ${bundle.description}\n`);
  ...
  process.stderr.write(
    `[replay] complete — ${bundle.calls.length} calls, ${failed} failures\n`,
  );
  return failed === 0 ? 0 : 1;
}
