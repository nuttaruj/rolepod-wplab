...
import { CompanionUnavailableError, ProductionBlockedError, WplabError } from '../util/errors.js'
...
export class CompanionBridge {
  ...
  async handshake(): Promise<HandshakeResponse> {
    ...
    if (res.status === 404) {
      throw new CompanionUnavailableError(this.target.id, 'companion plugin not installed or endpoints disabled')
    }
    if (res.status === 403) {
      throw new CompanionUnavailableError(this.target.id, 'companion refused — check Settings → WPLab Companion (enable endpoints + user has manage_options)')
    }
    if (res.status < 200 || res.status >= 300) {
      throw new CompanionUnavailableError(this.target.id, `handshake returned HTTP ${res.status}`)
    }

    const body = res.body as HandshakeResponse | null
    if (!body || typeof body !== 'object' || typeof body.session_token !== 'string') {
      throw new CompanionUnavailableError(this.target.id, 'handshake response missing session_token')
    }
  ...
  async executePhp(payload: string, opts: { timeoutMs?: number } = {}): Promise<ExecutePhpResponse> {
    // Pre-flight guards (Node side; companion re-screens server side)
    if (this.isProductionMatched) {
      throw new ProductionBlockedError(this.target.siteurl, 'companion reports prod match')
    }
  ...
  private coerceExecutePhpResponse(status: number, body: unknown): ExecutePhpResponse {
  ...
  async introspect(scope: string, opts: { includeValues?: boolean } = {}): Promise<IntrospectResponse> {
    const query: Record<string, string | number | boolean> = { scope }
    if (opts.includeValues !== undefined) query['include_values'] = opts.includeValues
    ...
    if (res.status === 404) {
      throw new CompanionUnavailableError(this.target.id, 'companion not installed')
    }
    if (res.status === 403) {
      // Production-blocked or admin-disabled
      const b = res.body as { error_code?: string; error_message?: string } | null
      if (b?.error_code === 'PRODUCTION_BLOCKED') {
        throw new ProductionBlockedError(this.target.siteurl, b.error_message ?? 'production')
      }
      throw new CompanionUnavailableError(this.target.id, b?.error_message ?? 'forbidden')
    }
    if (res.status < 200 || res.status >= 300) {
      throw new WplabError('INTROSPECT_FAILED', `introspect returned HTTP ${res.status}`, {
        status: res.status,
      })
    }
