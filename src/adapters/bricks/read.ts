...
export interface BricksReadAPI {
  ...
  listPages(target: Target, opts?: { type?: string; per_page?: number }): Promise<BricksPageSummary[]>
  ...
  async detect(target: Target): Promise<boolean> {
    ...
    try {
      ...
      if (body?.routes && typeof body.routes === 'object') {
        if (Object.keys(body.routes).some((r) => r.startsWith('/bricks/'))) return true
      }
    } catch {
      // fall through
    }
  ...
  read: {
    async listPages(target, opts = {}) {
      ...
      return items.map((raw) => {
        const p = raw as { id: number; title: { rendered: string }; status: string }
    ...
    async getPage(target, postId) {
      ...
      throw new Error('bricks.getPage on RestTarget requires companion v0.2 (post meta access).')
    },
  },
}
