...
export interface ElementorWriteAPI {
  ...
  updatePageData(target: Target, postId: number, widgetTree: unknown[]): Promise<{ bytesWritten: number; backupPath: string | null }>
}
  ...
  async updatePageData(target, postId, widgetTree) {
    ...
    try {
      ...
      if (result.exitCode !== 0) {
        throw new Error(`wp post meta update failed: ${result.stderr.slice(0, 200)}`)
      }
    } finally {
