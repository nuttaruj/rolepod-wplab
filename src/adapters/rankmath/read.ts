...
export const rankmathAdapter: Adapter<RankMathReadAPI> = {
  ...
  async detect(target: Target): Promise<boolean> {
    if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
      try {
        const r = await target.wpCli(['plugin', 'is-active', 'seo-by-rank-math'])
        if (r.exitCode === 0) return true
        const rPro = await target.wpCli(['plugin', 'is-active', 'seo-by-rank-math-pro'])
  ...
  read: {
    async postMeta(target, postId) {
      ...
      if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
        ...
        try {
          const rows = JSON.parse(r.stdout || '[]') as Array<{ meta_key: string; meta_value: string }>
    ...
    async settings(target) {
      if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
        const r = await target.wpCli(['option', 'get', 'rank-math-options-general', '--format=json'])
        if (r.exitCode === 0) {
          try {
