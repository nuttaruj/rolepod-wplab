...
export interface WooReadAPI {
  ...
  products(target: Target, opts?: { per_page?: number; search?: string }): Promise<unknown[]>

  /** List orders (read-only) via `/wc/v3/orders`. */
  orders(target: Target, opts?: { per_page?: number; status?: string }): Promise<unknown[]>
...
export const woocommerceAdapter: Adapter<WooReadAPI> = {
  ...
  read: {
    async products(target, opts = {}) {
      ...
      const res = await target.rest({ method: 'GET', path: '/wc/v3/products', query })
    ...
    async orders(target, opts = {}) {
      ...
      const res = await target.rest({ method: 'GET', path: '/wc/v3/orders', query })
      return Array.isArray(res.body) ? res.body : []
    },
    ...
    async shippingZones(target) {
      const res = await target.rest({ method: 'GET', path: '/wc/v3/shipping/zones' })
      return Array.isArray(res.body) ? res.body : []
    },

    async paymentGateways(target) {
      const res = await target.rest({ method: 'GET', path: '/wc/v3/payment_gateways' })
      return Array.isArray(res.body) ? res.body : []
    },
  },
}
