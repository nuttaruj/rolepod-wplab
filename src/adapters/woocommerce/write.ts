...
export interface WooWriteAPI {
  ...
  updateProduct(target: Target, id: number, fields: Record<string, unknown>): Promise<unknown>

  /** Bulk price update via REST batch endpoint. */
  bulkUpdatePrices(
    target: Target,
    updates: Array<{ id: number; regular_price?: string | undefined; sale_price?: string | undefined }>,
...
export const woocommerceWrite: WooWriteAPI = {
  ...
  async bulkUpdatePrices(target, updates) {
    ...
    return {
      ...
      failed: updates.length - (Array.isArray(body.update) ? body.update.length : 0),
    }
  },
}
