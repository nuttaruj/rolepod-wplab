...
export interface WpmlReadAPI {
  ...
  languages(
    target: Target,
  ): Promise<Array<{ code: string; native_name: string; default: boolean }>>;
  ...
  async detect(target: Target): Promise<boolean> {
    try {
      ...
      if (body?.routes && typeof body.routes === "object") {
        if (Object.keys(body.routes).some((r) => r.startsWith("/wpml/")))
          return true;
      }
    } catch {
      // fall through
  ...
  read: {
    async languages(target) {
      ...
      if (
        ...
        if (r.exitCode === 0) {
          try {
            const settings = JSON.parse(r.stdout || "{}") as {
            ...
            return Object.entries(active).map(([code, info]) => {
              ...
              return {
                code,
                native_name: i.native_name ?? code,
                default: code === def,
              };
            });
          } catch {
