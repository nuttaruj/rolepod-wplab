  ...
  async detect(target: Target): Promise<boolean> {
    try {
      ...
      if (body?.routes && typeof body.routes === 'object') {
        if (Object.keys(body.routes).some((r) => r.startsWith('/yoast/v1'))) return true
      }
    } catch {
      // fall through
    }
  ...
  read: {
    async postMeta(target, postId) {
      ...
      const body = (res.body ?? {}) as {
        id?: number
        yoast_head_json?: { description?: string; title?: string; canonical?: string; robots?: { index?: string } }
        meta?: Record<string, unknown>
      }
      const yhj = body.yoast_head_json ?? {}
      const meta = body.meta ?? {}
      const out: YoastPostMeta = { post_id: postId }
      if (typeof meta['_yoast_wpseo_focuskw'] === 'string') out.focus_keyword = meta['_yoast_wpseo_focuskw']
      if (typeof yhj.description === 'string') out.meta_description = yhj.description
    ...
    async settings(target) {
      if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
        const r = await target.wpCli(['option', 'get', 'wpseo_titles', '--format=json'])
        if (r.exitCode === 0) {
          try {
