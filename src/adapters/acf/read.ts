...
export const acfAdapter: Adapter<AcfReadAPI> = {
  ...
  async detect(target: Target): Promise<boolean> {
    ...
    try {
      ...
      if (body?.routes && typeof body.routes === 'object') {
        if (Object.keys(body.routes).some((r) => r.startsWith('/acf/'))) return true
      }
    } catch {
      // fall through
    }
    ...
    if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
      try {
        const r = await target.wpCli(['plugin', 'is-active', 'advanced-custom-fields'])
        if (r.exitCode === 0) return true
        const rPro = await target.wpCli(['plugin', 'is-active', 'advanced-custom-fields-pro'])
  ...
  read: {
    async fieldGroups(target) {
      ...
      const res = await target.rest({ method: 'GET', path: '/acf/v3/field-groups' })
      if (res.status >= 200 && res.status < 300 && Array.isArray(res.body)) {
        return (res.body as Array<{ id: number; title: string; key: string; active?: boolean }>).map(
          (g) => ({ id: g.id, title: g.title, key: g.key, active: g.active ?? true }),
        )
      }
      return []
    },
    ...
    async postMeta(target, postId) {
      ...
      if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
        ...
        if (r.exitCode !== 0) return {}
        const all = JSON.parse(r.stdout || '[]') as Array<{ meta_key: string; meta_value: string }>
        const acfOnly: Record<string, unknown> = {}
        for (const row of all) {
          if (row.meta_key.startsWith('_') || row.meta_key.startsWith('field_')) continue
