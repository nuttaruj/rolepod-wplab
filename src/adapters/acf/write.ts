...
export interface AcfWriteAPI {
   ...
   */
  setPostMeta(target: Target, postId: number, fieldName: string, value: unknown): Promise<{ source: 'rest_acf_pro' | 'wp_cli' }>
}
  ...
  async setPostMeta(target, postId, fieldName, value) {
    ...
    if (target.kind === 'local' || target.kind === 'ssh' || target.kind === 'docker') {
      ...
      const r = await target.wpCli(
        ['post', 'meta', 'update', String(postId), fieldName, stringified, '--format=json'],
        { allowDestructive: true },
      )
      if (r.exitCode !== 0) {
        throw new Error(`acf.setPostMeta wp-cli fallback failed: ${r.stderr.slice(0, 200)}`)
      }
