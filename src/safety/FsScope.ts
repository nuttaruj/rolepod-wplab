...
export function resolveScopedWrite(
  ...
  const inScopedDir = SCOPED_DIRS.some(
    (dir) => rel === dir || rel.startsWith(`${dir}${sep}`),
  );
  if (!inScopedDir) {
    if (confirmUnsafePath) return realCandidate;
    throw new FsScopeError(
      candidate,
      `not under ${SCOPED_DIRS.join(" / ")} or wp-config.php`,
    );
  }
  ...
  if (rel.startsWith(`wp-content/uploads${sep}`)) {
    ...
    if (!UPLOAD_EXT_ALLOWLIST.has(ext)) {
      throw new FsScopeError(
        candidate,
        `extension ${ext || "<none>"} not allowed in uploads/`,
      );
    }
  }
