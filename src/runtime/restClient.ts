...
export class RestClient {
  ...
  constructor(opts: RestClientOptions) {
    ...
    if (u.protocol !== 'https:') {
      throw new WplabError('REST_REQUIRES_HTTPS', 'RestTarget refuses non-https:// URLs', {
    ...
    this.authHeader =
      'Basic ' +
      Buffer.from(`${opts.credential.username}:${opts.credential.appPassword}`).toString('base64')
    this.userAgent = opts.userAgent ?? `rolepod-wplab/0.1 (+https://github.com/nuttaruj/rolepod-wplab)`
    this.defaultTimeout = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS
  }

  async request(req: RestRequestInit): Promise<RestResponseRaw> {
    const path = req.path.startsWith('/') ? req.path : `/${req.path}`
    const queryParams = req.query ? new URLSearchParams(stringifyQuery(req.query)) : null
  ...
  private async send(url: string, req: RestRequestInit): Promise<RestResponseRaw> {
    ...
    try {
      ...
      const parsed: unknown = contentType.includes('application/json') && text.length > 0
        ? safeJsonParse(text)
        : text
      ...
      if (e.name === 'AbortError') {
        throw new WplabError('REST_TIMEOUT', `REST request timed out after ${req.timeoutMs ?? this.defaultTimeout}ms`, {
          url: redactUrl(url),
        })
      }
      throw new WplabError('REST_NETWORK_ERROR', `REST request failed: ${e.message ?? 'unknown'}`, {
        url: redactUrl(url),
      })
    } finally {
  ...
  private looksLikePermalinkOff(response: RestResponseRaw): boolean {
    ...
    if (typeof response.body === 'string') {
      return /no route was found/i.test(response.body) || /<!doctype html>/i.test(response.body)
    }
...
function stringifyQuery(q: Record<string, string | number | boolean>): Record<string, string> {
