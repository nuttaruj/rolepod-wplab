...
export class DockerTarget implements Target {
  ...
  static async open(opts: DockerTargetOptions): Promise<DockerTarget> {
    ...
    const version = await execInside(docker, opts.containerName, ['wp', `--path=${opts.wpPath}`, '--no-color', 'core', 'version'])
    ...
    const siteurl = await execInside(docker, opts.containerName, ['wp', `--path=${opts.wpPath}`, '--no-color', 'option', 'get', 'siteurl'])

    log.info('DockerTarget opened', { container: opts.containerName, wpPath: opts.wpPath })
    ...
    return new DockerTarget({
      ...
      wpPath: opts.wpPath,
      siteurl: siteurl.stdout.trim() || `docker://${opts.containerName}${opts.wpPath}`,
  ...
  async fileRead(path: string): Promise<{ content: string; bytes: number; absolutePath: string }> {
    ...
    if (r.exitCode !== 0) {
      throw new WplabError('FILE_READ_FAILED', `cat ${abs} exit ${r.exitCode}`, {})
    }
    return { content: r.stdout, bytes: Buffer.byteLength(r.stdout, 'utf8'), absolutePath: abs }
  }
  ...
  async fileWrite(
    ...
    const r = await execInside(
      this._docker,
      this._containerName,
      ['sh', '-c', `mkdir -p "$(dirname ${shQuote(abs)})" && cat > ${shQuote(abs)}`],
      content,
    )
    if (r.exitCode !== 0) {
      throw new WplabError('FILE_WRITE_FAILED', `tee ${abs} exit ${r.exitCode}`, {})
    }
    return { bytesWritten: Buffer.byteLength(content, 'utf8'), backupPath: null, absolutePath: abs }
  }

  async fileExists(path: string): Promise<boolean> {
    const r = await execInside(this._docker, this._containerName, ['test', '-f', `${this._wpPath}/${path}`])
    return r.exitCode === 0
  }
...
async function execInside(
  ...
  const stream = await exec.start({ hijack: stdin !== undefined, stdin: stdin !== undefined })
