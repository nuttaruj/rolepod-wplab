...
export class RestTarget implements Target {
   ...
   */
  static async open(args: { url: string; credential: Credential }): Promise<RestTarget> {
    const client = new RestClient({ baseUrl: args.url, credential: args.credential })

    // Probe auth + REST reachability
    const probe = await client.request({ method: 'GET', path: '/wp/v2/types', timeoutMs: 10_000 })
    ...
    // Pull site metadata
    const meta = await client.request({ method: 'GET', path: '/', timeoutMs: 10_000 })
  ...
  async wpCli(_args: readonly string[], _opts?: WpCliOpts): Promise<WpCliResult> {
    throw new WplabError(
  ...
  async fileRead(_path: string): Promise<{ content: string; bytes: number; absolutePath: string }> {
    throw new WplabError(
      'COMPANION_REQUIRED_V0_2',
