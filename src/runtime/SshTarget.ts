...
export class SshTarget implements Target {
  ...
  static async open(opts: SshTargetOptions): Promise<SshTarget> {
    ...
    if (opts.privateKeyPath) {
      ...
      throw new WplabError(
        "SSH_NO_AUTH",
        "SshTarget requires privateKeyPath or password",
        {},
      );
    }
    ...
    const version = await ssh.execCommand(
      `wp --path=${escapeArg(opts.wpPath)} core version --no-color`,
    );
    if (version.code !== 0) {
      ssh.dispose();
      throw new WplabError(
        "SSH_WP_PROBE_FAILED",
        `wp core version exit ${version.code}: ${version.stderr.slice(0, 200)}`,
        {
          host: opts.host,
        },
      );
    }
    const siteurlOut = await ssh.execCommand(
      `wp --path=${escapeArg(opts.wpPath)} option get siteurl --no-color`,
    );
  ...
  async fileExists(path: string): Promise<boolean> {
    const abs = `${this._wpPath}/${path}`;
    const r = await this._ssh.execCommand(
      `test -f ${escapeArg(abs)} && echo yes`,
    );
